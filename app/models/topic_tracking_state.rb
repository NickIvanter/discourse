...
class TopicTrackingState
  ...
  def self.report(user, topic_id = nil)
    ...
    sql << "\nUNION ALL\n\n"
    sql << report_raw_sql(topic_id: topic_id, skip_new: true, skip_order: true, staff: user.staff?, guardian: guardian)
  ...
  def self.report_raw_sql(opts=nil)
    ...
    unread =
      if opts && opts[:skip_unread]
        ...
        TopicQuery.unread_filter(Topic, -999, staff: opts && opts[:staff], guardian: guardian).where_values.join(" AND ").sub("-999", ":user_id")
      end
