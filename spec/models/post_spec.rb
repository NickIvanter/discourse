...
describe Post do
  ...
  describe 'scopes' do
    ...
    describe '#cloak_stealth' do
      ...
      let(:author)   { topic.user }
      let(:admin)    { Fabricate(:user, admin: true) }

      let!(:p1) { Post.create(topic: topic, user: author, post_number: 1, raw: 'Test post 1') }
      let!(:p2) { Post.create(topic: topic, user: writer, post_number: 2, raw: 'Test post 2') }
      let!(:p3) { Post.create(topic: topic, user: writer, post_number: 3, raw: 'Test post 3', reply_to_post_number: p1.post_number) }
      ...
      context 'when stealth approving on' do
        ...
        context 'when stealth posts present' do
          ...
          it 'cloak reply history to post author' do
            ...
            expect(p4.reply_history(100, Guardian.new(writer))).to eq([])
          end
